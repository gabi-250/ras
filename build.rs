...
fn generate_instruction_repr() {
    ...
    let content = quote! {
        ...
        use crate::x86::repr::{InstructionRepr, OperandRepr, OperandKind};
    ...
    let path = format!(
        "{}/src/x86/repr/instruction_map.rs",
...
fn build_operand_enc(operand: &str, size: usize) -> TokenStream {
    if MODRM_REG_RE.is_match(operand) {
        return quote! { Some(OperandRepr::new(OperandKind::ModRmReg, #size)) };
    } else if MODRM_RM_RE.is_match(operand) {
        return quote! { Some(OperandRepr::new(OperandKind::ModRmRegMem, #size)) };
    } else if ALL_ACC_RE.is_match(operand) {
        return quote! { Some(OperandRepr::new(OperandKind::Al, #size)) };
    } else if ACC_OVER_16_RE.is_match(operand) {
        return quote! { Some(OperandRepr::new(OperandKind::Al, #size)) };
    } else if IMM_RE.is_match(operand) {
        return quote! { Some(OperandRepr::new(OperandKind::Imm, #size)) };
    } else if IMM8_RE.is_match(operand) {
        return quote! { Some(OperandRepr::new(OperandKind::Imm, #size)) };
    } else if IW_RE.is_match(operand) || operand == "imm16" {
        return quote! { Some(OperandRepr::new(OperandKind::Imm, #size)) };
    } else if OPCODE_RD_RE.is_match(operand) {
        return quote! { Some(OperandRepr::new(OperandKind::OpcodeRd, #size)) };
    } else if MOFFS_RE.is_match(operand) {
        return quote! { Some(OperandRepr::new(OperandKind::MemoryOffset, #size)) };
    } else if operand == "1" {
        return quote! { Some(OperandRepr::new(OperandKind::One, #size)) }; // XXX
    } else if operand == "CL" {
        return quote! { Some(OperandRepr::new(OperandKind::Cl, #size)) }; // XXX
